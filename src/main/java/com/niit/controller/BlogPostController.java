...
private UserDao userDao;
     ...
     public ResponseEntity<?> saveBlogPost(@RequestBody BlogPost blogPost, HttpSession session){
    	 ...
    	 if(session.getAttribute("username")==null){
    		 ...
    		 return new ResponseEntity<Error>(error,HttpStatus.UNAUTHORIZED);// status code 401
    	 }
    	 String username=(String)session.getAttribute("username");
    	 User user=userDao.getUserByUsername(username);
    	 blogPost.setPostedOn(new Date()); 
 //   	 System.out.println("Date is checked" +(new Date()));
    	 blogPost.setPostedBy(user);
    	 
    	 try{
    		 blogPostDao.saveBlogPost(blogPost);
    		 return new ResponseEntity<BlogPost>(blogPost,HttpStatus.OK);// 200
    	 }catch(Exception e){
 	...
 	public ResponseEntity<?> addBlogComment(@RequestBody BlogComment blogComment,HttpSession session){
 		...
 		blogComment.setCommentedOn(new Date());//set this value for commentOn
 		try{
